...
export default function Sidebar() {
  ...
  return (
    <div
      className={`fixed top-32 sm:top-16 left-0 h-[calc(101vh-135px)] sm:h-[calc(100vh-76px)] w-64 bg-slate-50/95 backdrop-blur-md text-gray-900 flex flex-col border-r border-gray-200/80 shadow-2xl z-40 transition-transform duration-300 ${
        menu ? "translate-x-0" : "-translate-x-full md:translate-x-0"
      }`}
    >
      {/* 1. USER METADATA HEADER */}
      <div className="pt-4 pb-4 px-6 border-b border-gray-200/60 bg-white shrink-0">
        <span className="text-[10px] font-bold uppercase tracking-widest text-indigo-600 block">
          Authorized Account
        </span>
        <h2 className="text-base font-extrabold text-gray-900 tracking-tight mt-0.5 truncate">
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto pt-4">
        <NavLink
